...
function getDirectoryConstructorsAt(path) {
  const keys = Object.keys(QmlWeb.qrc).filter(key => key.startsWith(path) && key.match(/\.qml$/i));
  const constructors = [];
  const QMLComponent = QmlWeb.getConstructor("QtQml", "2.0", "Component");
...
// eslint-disable-next-line no-undef
class QtQml_QValidator extends QtQml_QtObject {

  fixup() {} // stub, does nothing
}
...
class QtQuick_Gradient extends QtQml_QtObject {
  ...
  $onGradientStopChanged() {
    ...
    if (this.stops.length > 0) {
      ...
      const direction = this.orientation === this.Orientation.Vertical ? "to bottom" : "to right";
...
class QtQuick_Layouts_ColumnLayout extends QtQuick_Layouts_DirectionalLayout {
  ...
  $fillLayoutCells(layout) {
    const extendedItems = layout.fillColumnCount > 0 ? layout.fillColumnCount : layout.cells.length;
    const extraHeight = (this.implicitHeight - layout.contentHeight) / extendedItems;
    let offsetY = 0;
...
class QtQuick_Layouts_GridLayout extends QtQuick_Layouts_Positioner {
  ...
  $setCellPreferredGeometry(layout) {
    for (let x = 0; x < layout.columns; ++x) {
      for (let y = 0; y < layout.rows; ++y) {
        ...
        if (cell) {
          const vMargin = this.$inferCellMargin(cell.occupant, "top") + this.$inferCellMargin(cell.occupant, "bottom");
          const hMargin = this.$inferCellMargin(cell.occupant, "left") + this.$inferCellMargin(cell.occupant, "right");
      ...
      for (let x = 0; x < layout.columns; ++x) {
        ...
        if (child) {
          ...
          for (let i = 0; i < child.$Layout[`${attr}Span`] - 1; ++i) {
            const absorbedCell = direction === "Width" ? this.$getCellDescriptor(layout, x + 1, y) : this.$getCellDescriptor(layout, x, y + 1);
  ...
  $alignItemInCell(cell, rtl) {
    let alignment = cell.item.$Layout.alignment !== null ? cell.item.$Layout.alignment : QmlWeb.Qt.AlignVCenter | QmlWeb.Qt.AlignLeft;
...
class QtQuick_Layouts_RowLayout extends QtQuick_Layouts_DirectionalLayout {
  ...
  $fillLayoutCells(layout) {
    const extendedItems = layout.fillRowCount > 0 ? layout.fillRowCount : layout.cells.length;
    const extraWidth = (this.implicitWidth - layout.contentWidth) / extendedItems;
    let offsetX = 0;
...
class QtQuick_ColorAnimation extends QtQuick_PropertyAnimation {
  ...
  $startLoop() {
    for (const i in this.$actions) {
      ...
      const from = action.from || action.target[action.property] || EMPTY_ANIMATION_COLOR;
      const to = action.to || action.target[action.property] || EMPTY_ANIMATION_COLOR;
...
class QtQuick_Text extends QtQuick_Item {
  ...
  constructor(meta) {
    ...
    this.verticalAlignmentChanged.connect(this, this.$onVerticalAlignmentChanged);
    this.styleChanged.connect(this, this.$onStyleChanged);
